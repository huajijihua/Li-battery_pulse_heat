...
function updateAnnotation(mcbSubsys)
    ...
    text = "V4-B3 MCB FOC controller boundary" + newline + ...
        ...
        "Gate: Duty_abc(3x1) -> threshold comparison -> DTC(double) -> complementary -> 6 gate -> UnitDelay -> Duty_Cycles" + newline + ...
        "Params: V4 drive (N=6, PM=0.03, Rs=0.013, 48V) overrides MCB defaults; PI gains recalculated in InitFcn" + newline + ...
        "DataStores: Enable=1, Speed_ref=0 (stall)" + newline + ...
        "ControlKPI = [tracking_NaN; modindex_NaN; sat_NaN; enable=1; status=3]. V4-D2.2 restore replaces threshold with PWM carrier and numeric duty KPI.";
