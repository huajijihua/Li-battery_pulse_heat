...
fprintf('=== 双电机同相零维脉冲加热模型: 高频析锂自适应限流版 ===\n');
...
[p, USE_DEMO_VALUES] = build_3806_platform_params('dual_inphase_lowtemp_window');
...
fprintf('限流模型: %s | 规格书回充%.0fA, 高频析锂回充%.0fA\n', ...
    op_dual.current_limit_model, op_dual.I_spec_charge_limit, op_dual.I_plating_charge_limit);
...
sgtitle(sprintf('图1: 双电机同相整车电池组实际脉冲电流 - 低温窗口限流 @ T_{init}=%.0f°C', p.T_init), ...
    'FontWeight', 'bold');
...
for idx_T = 1:numel(T_scan)
    ...
    title(sprintf('图%s: 原始整车电池组 I_{rms} + 约束边界 @ %.0f°C, 双电机同相', fig2_labels{idx_T}, T_cur));
...
sgtitle(sprintf('图3: 净加热判据与温升速率 - 低温窗口限流 (T_{amb}=%.0f°C)', p.T_amb), ...
    'FontWeight', 'bold');
...
sgtitle(sprintf('图4: 双电机同相全动态时域仿真 - 低温窗口限流 @ T_{init}=%.0f°C (%.0fmin)', ...
...
pie(max(dual_energy.values, 0.001), dual_energy.labels);
title({sprintf('图5: %.0fmin能量分配 (双电机同相, 低温窗口限流)', p.t_end_min), ...
    sprintf('总电耗=%.2fMJ, 系统加热效率=%.1f%%', dual_energy.total_MJ, dual_energy.eta_sys)}, ...
...
for k = 1:numel(T_eff_scan)
    ...
    eta_dual(k) = op2.P_bat / max(P_total_diag_2, eps) * 100;
    soc_per_deg_dual(k) = (P_total_diag_2 / (p.E_pack_kWh * 3.6e6)) / ...
...
plot(T_label_points, interp1(T_eff_scan, eta_dual, T_label_points), 'o', ...
    'Color', [0.85 0.20 0.20], 'MarkerFaceColor', 'w', 'LineWidth', 1.4);
for idx_label = 1:numel(T_label_points)
    T_label = T_label_points(idx_label);
    eta_label = interp1(T_eff_scan, eta_dual, T_label);
    text(T_label, eta_label, sprintf('  %.1f%%', eta_label), ...
...
plot(T_label_points, interp1(T_eff_scan, soc_per_deg_dual, T_label_points), 'o', ...
    'Color', [0.00 0.30 0.85], 'MarkerFaceColor', 'w', 'LineWidth', 1.4);
for idx_label = 1:numel(T_label_points)
    T_label = T_label_points(idx_label);
    soc_label = interp1(T_eff_scan, soc_per_deg_dual, T_label);
    text(T_label, soc_label, sprintf('  %.3f%%/°C', soc_label), ...
        'Color', [0.00 0.20 0.65], 'FontSize', 9, 'FontWeight', 'bold', ...
        'VerticalAlignment', 'bottom');
end
...
sgtitle(sprintf('图5b: 加热效率与SOC电耗 - 低温窗口限流 (f=%.0fHz, L_d=%.1fuH)', ...
...
title(sprintf('图6: 双电机同相可行性边界 - 电器限/析锂限 (T_{init}=%.0f°C, T_{amb}=%.0f°C, SOC_0=%.0f%%)', ...
...
xlabel('开关频率 f (Hz)'); ylabel('D轴电感 L_d (mH)');
title(sprintf('图6b: 原始温升能力背景 + 电器限/析锂限 (T_{init}=%.0f°C, T_{amb}=%.0f°C, SOC_0=%.0f%%)', ...
    p.T_init, p.T_amb, p.SOC_init*100), 'FontWeight', 'bold');
xlim([min(f_bnd), max(f_bnd)]);
ylim(p.fig6_L_display_mH);
set(gca, 'XTick', [100 500 1000 1500 2000 2500 3000]);
...
fprintf('口径: 公共电池/直流母线, 两条同相支路; 电池发热按整车电池组总电流, 电池侧限流模式=%s。\n', ...
