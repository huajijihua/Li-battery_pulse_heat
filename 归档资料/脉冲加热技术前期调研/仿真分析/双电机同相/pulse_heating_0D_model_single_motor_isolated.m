...
%   单台 CAM255PT52 电机参与脉冲加热, 另一台电机与电池回路断开。
%
% 建模目的:
%   作为双电机同相模型的对比工况: 整车电池组只连接一条
%   “逆变器-电机绕组”支路。
%
% 单电机假设:
%   只有一台逆变器输出PWM电压, 另一台电机不参与电池加热回路。
...
%
% 注意:
%   本脚本用于单电机连通、另一电机断开时的对比计算。
%   因此后处理必须同时区分:
%     1) 单支路变量 vs 整车电池组总量
%     2) 整车电池组总电流 vs 单台电机支路电流
%   本脚本已切换为3806整车平台参数。电池串并联、能量、质量、内阻、
%   30s/60s低温电流窗口均来自参数汇总表中的实车/实验数据, 并作为
%   低频/准直流参考边界保留。高频脉冲加热默认使用负极极化析锂
%   判据给出电池侧回充半周限流。
...
[p, USE_DEMO_VALUES] = build_3806_platform_params('single_motor_isolated');
...
op_single = calc_operating_point(p, p.motor_count, p.T_init, p.SOC_init);

print_operating_point('单电机连通', op_single);
fprintf('限流模型: %s | 规格书回充%.0fA, 高频析锂回充%.0fA\n', ...
    op_single.current_limit_model, op_single.I_spec_charge_limit, op_single.I_plating_charge_limit);

[t_wave, i_branch_wave_unlimited, v_pwm_wave] = calc_pwm_waveform( ...
    p.V_pack_nom, op_single.R_branch_equiv, p.L_d, p.f_sw, p.D, 600);
i_branch_wave = i_branch_wave_unlimited * op_single.current_scale;
...
yline(op_single.I_pack_rms, 'k--', sprintf('整车电池组RMS %.0fA', op_single.I_pack_rms), 'LineWidth', 1.3);
yline(op_single.I_pack_peak, 'r--', sprintf('整车电池组峰值 %.0fA', op_single.I_pack_peak), 'LineWidth', 1.2);
yline(-op_single.I_pack_peak, 'r--', '', 'LineWidth', 1.2);
yline(0, 'k-', 'LineWidth', 0.5);
xlabel('时间 (ms)'); ylabel('电流 (A)');
title('整车电池组实际脉冲电流');
ylim(padded_limits([i_pack_wave(:); op_single.I_pack_peak; -op_single.I_pack_peak], 0.10, true));
...
sgtitle(sprintf('图1: 单电机连通整车电池组实际脉冲电流 @ T_{init}=%.0f°C', p.T_init), ...
    'FontWeight', 'bold');
...
fig2_ids = [21 22 23];
fig2_labels = {'2a', '2b', '2c'};

for idx_T = 1:numel(T_scan)
    T_cur = T_scan(idx_T);
    I_pack_rms_raw_grid = zeros(size(F_grid));
    I_pack_peak_raw_grid = zeros(size(F_grid));
    I_plating_charge_grid = zeros(size(F_grid));
    I_branch_rms_grid = zeros(size(F_grid));
    I_branch_peak_grid = zeros(size(F_grid));
    electrical_limit_grid = zeros(size(F_grid));
    plating_limit_grid = zeros(size(F_grid));
    dTdt_raw_grid = zeros(size(F_grid));
    ...
    for i = 1:size(F_grid, 1)
        for j = 1:size(F_grid, 2)
            ...
            I_pack_rms_raw_grid(i,j) = op_ij.I_pack_rms_raw;
            I_pack_peak_raw_grid(i,j) = op_ij.I_pack_peak_raw;
            I_plating_charge_grid(i,j) = op_ij.I_plating_charge_limit;
            ...
            plating_limit_grid(i,j) = op_ij.I_pack_peak_raw / max(op_ij.I_plating_charge_limit, eps);
            P_bat_raw = op_ij.I_pack_rms_raw^2 * op_ij.R_bat_eff;
            dTdt_raw_grid(i,j) = (P_bat_raw - op_ij.P_loss) / p.Cth_bat * 60;
        end
    end

    figure(fig2_ids(idx_T)); clf;
    set(gcf, 'Position', [120 + 70*(idx_T-1), 120 + 40*(idx_T-1), 980, 720], 'Color', 'w');
    I_pack_rms_plot = min(I_pack_rms_raw_grid, p.fig2_I_pack_rms_clim(2));
    contourf(F_grid, L_grid*1000, I_pack_rms_plot, ...
        0:100:p.fig2_I_pack_rms_clim(2), ...
        'LineColor', 'none');
    colormap(gca, parula(256));
    cb = colorbar; ylabel(cb, '原始整车电池组 I_{rms} (A)');
    ...
    contour(F_grid, L_grid*1000, dTdt_raw_grid, [1.0 1.0], ...
        '--', 'Color', [0.00 0.95 0.15], 'LineWidth', 2.2);
    contour(F_grid, L_grid*1000, dTdt_raw_grid, [0.5 0.5], ...
    ...
    contour(F_grid, L_grid*1000, plating_limit_grid, [1.0 1.0], ...
        '-', 'Color', [1.00 0.00 1.00], 'LineWidth', 2.6);
    ...
    title(sprintf('图%s: 原始整车电池组 I_{rms} + 约束边界 @ %.0f°C, 单电机连通', fig2_labels{idx_T}, T_cur));
    ...
    leg_labels{1} = '电器限';
    h_leg(2) = plot(nan, nan, '-', 'Color', [1.00 0.00 1.00], 'LineWidth', 2.4);
    leg_labels{2} = '析锂限';
    h_leg(3) = plot(nan, nan, '--', 'Color', [0.00 0.95 0.15], 'LineWidth', 2.2);
    leg_labels{3} = '1°C/min';
    h_leg(4) = plot(nan, nan, '--', 'Color', [0.00 0.95 1.00], 'LineWidth', 2.0);
    leg_labels{4} = '0.5°C/min';
    h_leg(5) = h_limit;
    leg_labels{5} = 'f上限';
    h_leg(6) = h_cur;
    leg_labels{6} = '当前';
...
plot(T_range, P_net_curve/1000, '--', 'LineWidth', 1.8, ...
    'Color', [0.00 0.55 0.20], 'DisplayName', '单电机净加热 P_{net}');
plot(T_range, P_motor_curve/1000, '-.', 'LineWidth', 1.5, ...
    'Color', [0.90 0.60 0.05], 'DisplayName', '电机损耗 P_{motor}');
plot(T_range, P_inv_curve/1000, '-.', 'LineWidth', 1.5, ...
    'Color', [0.55 0.10 0.80], 'DisplayName', '控制器损耗 P_{inv}');
...
plot(T_range, dTdt_curve, 'LineWidth', 2.0, ...
    'Color', [0.85 0.20 0.20], 'DisplayName', '单电机连通');
...
sim_single = simulate_case(p, p.motor_count);

fprintf('\n[时域结果]\n');
fprintf('%.0fmin: T=%.2f°C, SOC=%.2f%%\n', ...
    p.t_end_min, sim_single.T(end), sim_single.SOC(end)*100);
fprintf('达到%.0f°C时间: %s\n', ...
    p.T_target, format_target_time(sim_single.t, sim_single.T, p.T_target));

figure(4); clf;
set(gcf, 'Position', [80 60 1280 850], 'Color', 'w');

subplot(3,2,1); hold on;
plot(sim_single.t/60, sim_single.T, 'Color', [0.85 0.20 0.20], 'LineWidth', 1.9);
yline(p.T_target, 'k--', sprintf('目标 %.0f°C', p.T_target));
xlabel('时间 (min)'); ylabel('电池温度 (°C)');
title('温度轨迹');
legend('单电机连通', 'Location', 'best');
grid on;
xlim(padded_limits(sim_single.t/60, 0.03, false));
ylim(padded_limits([sim_single.T; p.T_target], 0.08, true));
hold off;

subplot(3,2,2); hold on;
plot(sim_single.t/60, sim_single.dTdt, 'Color', [0.85 0.20 0.20], 'LineWidth', 1.8);
yline(1.0, 'r--', '1°C/min');
...
grid on;
xlim(padded_limits(sim_single.t/60, 0.03, false));
ylim(padded_limits([sim_single.dTdt; 1.0; 0.5], 0.12, true));
hold off;

subplot(3,2,3); hold on;
h_cur = [];
cur_labels = {};
h_cur(end+1) = plot(sim_single.t/60, sim_single.I_branch_peak, 'b-', 'LineWidth', 1.7);
cur_labels{end+1} = '单电机支路峰值';
h_cur(end+1) = plot(sim_single.t/60, sim_single.I_branch_rms, '-', 'Color', [0.00 0.60 0.20], 'LineWidth', 1.7);
cur_labels{end+1} = '单电机支路RMS';
h_cur(end+1) = plot(sim_single.t/60, sim_single.I_pack_peak, 'r-', 'LineWidth', 1.7);
cur_labels{end+1} = '整车电池组峰值';
h_cur(end+1) = plot(sim_single.t/60, sim_single.I_pack_rms, 'k--', 'LineWidth', 1.4);
...
grid on;
xlim(padded_limits(sim_single.t/60, 0.03, false));
ylim(padded_limits([sim_single.I_branch_rms; sim_single.I_branch_peak; ...
    sim_single.I_pack_peak; sim_single.I_pack_rms; p.I_motor_rms_limit; ...
    p.I_motor_peak_limit], 0.08, true));
hold off;

subplot(3,2,4); hold on;
plot(sim_single.t/60, sim_single.P_bat/1000, '-', 'Color', [0.85 0.20 0.20], 'LineWidth', 1.8);
plot(sim_single.t/60, sim_single.P_loss/1000, '-', 'Color', [0.05 0.05 0.05], 'LineWidth', 2.0);
plot(sim_single.t/60, sim_single.P_motor/1000, '-', 'Color', [0.90 0.60 0.05], 'LineWidth', 1.6);
plot(sim_single.t/60, sim_single.P_inv/1000, '-', 'Color', [0.55 0.10 0.80], 'LineWidth', 1.6);
plot(sim_single.t/60, sim_single.P_net/1000, '--', 'Color', [0.00 0.55 0.20], 'LineWidth', 1.8);
xlabel('时间 (min)'); ylabel('功率 (kW)');
title('单电机连通功率分解');
legend('P_{bat}', 'P_{loss}', 'P_{motor}', 'P_{inv}', 'P_{net}', 'Location', 'best');
grid on;
xlim(padded_limits(sim_single.t/60, 0.03, false));
ylim(padded_limits([sim_single.P_bat; sim_single.P_loss; ...
    sim_single.P_motor; sim_single.P_inv; sim_single.P_net]/1000, 0.12, true));
hold off;

subplot(3,2,5);
yyaxis left;
plot(sim_single.t/60, sim_single.P_net/1000, '-', 'Color', [0.00 0.55 0.20], 'LineWidth', 1.7);
ylabel('净加热功率 (kW)');
yyaxis right;
plot(sim_single.t/60, sim_single.E_cum_MJ, '-', 'Color', [0.85 0.33 0.10], 'LineWidth', 1.7);
ylabel('累计电耗 (MJ)');
xlabel('时间 (min)');
title('净加热功率与累计电耗');
grid on;
...
subplot(3,2,6);
yyaxis left;
plot(sim_single.t/60, sim_single.SOC*100, '-', 'Color', [0.00 0.30 0.85], 'LineWidth', 1.7);
ylabel('SOC (%)');
yyaxis right;
plot(sim_single.t/60, sim_single.T - p.T_init, '-', 'Color', [0.85 0.33 0.10], 'LineWidth', 1.7);
ylabel('累计温升 (°C)');
xlabel('时间 (min)');
title('SOC消耗与累计温升');
grid on;

sgtitle(sprintf('图4: 单电机连通全动态时域仿真 @ T_{init}=%.0f°C (%.0fmin)', ...
...
single_energy = summarize_energy(sim_single, p);

pie(max(single_energy.values, 0.001), single_energy.labels);
title({sprintf('图5: %.0fmin能量分配 (单电机连通)', p.t_end_min), ...
    sprintf('总电耗=%.2fMJ, 系统加热效率=%.1f%%', single_energy.total_MJ, single_energy.eta_sys)}, ...
...
T_eff_scan = -20:2:10;
eta_single = zeros(size(T_eff_scan));
soc_per_deg_single = zeros(size(T_eff_scan));

for k = 1:numel(T_eff_scan)
    op2 = calc_operating_point(p, p.motor_count, T_eff_scan(k), p.SOC_init);
    P_motor_diag_2 = op2.motor_count * op2.I_branch_rms^2 * p.R_s;
    P_inv_diag_2 = op2.motor_count * calc_inverter_loss(p, op2.I_branch_rms, p.f_sw, p.V_pack_nom);
    P_total_diag_2 = op2.P_bat + P_motor_diag_2 + P_inv_diag_2;
    eta_single(k) = op2.P_bat / max(P_total_diag_2, eps) * 100;
    soc_per_deg_single(k) = (P_total_diag_2 / (p.E_pack_kWh * 3.6e6)) / ...
...
subplot(2,1,1); hold on;
plot(T_eff_scan, eta_single, 'Color', [0.85 0.20 0.20], 'LineWidth', 1.8);
xlabel('电池温度 (°C)'); ylabel('电气加热效率 (%)');
title('诊断电气加热效率 = 电池发热 / (电池+铜耗+逆变器损耗)');
legend('单电机连通', 'Location', 'best');
grid on;
xlim(padded_limits(T_eff_scan, 0.03, false));
ylim(padded_limits(eta_single(:), 0.08, false));
hold off;

subplot(2,1,2); hold on;
plot(T_eff_scan, soc_per_deg_single, 'Color', [0.00 0.30 0.85], 'LineWidth', 1.8);
xlabel('电池温度 (°C)'); ylabel('SOC消耗 (%/°C)');
title('诊断每升高1°C所需SOC消耗');
legend('单电机连通', 'Location', 'best');
grid on;
xlim(padded_limits(T_eff_scan, 0.03, false));
ylim(padded_limits(soc_per_deg_single(:), 0.08, false));
hold off;

sgtitle(sprintf('图5b: 加热效率与SOC电耗 (f=%.0fHz, L_d=%.1fuH)', ...
...
for i = 1:size(F_B, 1)
    for j = 1:size(F_B, 2)
        ...
        P_bat_raw = op_ij.I_pack_rms_raw^2 * op_ij.R_bat_eff;
        dTdt_raw_map(i,j) = (P_bat_raw - op_ij.P_loss) / p.Cth_bat * 60;
        I_branch_rms_map(i,j) = op_ij.I_branch_rms_raw;
        I_branch_peak_map(i,j) = op_ij.I_branch_peak_raw;
        electrical_limit_map(i,j) = max( ...
            op_ij.I_branch_rms_raw / p.I_motor_rms_limit, ...
            op_ij.I_branch_peak_raw / p.I_motor_peak_limit);
        plating_limit_map(i,j) = op_ij.I_pack_peak_raw / max(op_ij.I_plating_charge_limit, eps);
    end
end
...
if min(plating_limit_map(:)) <= 1.0 && max(plating_limit_map(:)) >= 1.0
    [~, h_plating] = contour(F_B, L_B*1000, plating_limit_map, [1.0 1.0], ...
        '-', 'Color', [1.00 0.00 1.00], 'LineWidth', 3.0);
    n_lines = n_lines + 1;
    h_lines(n_lines) = h_plating;
    leg_labels{n_lines} = '析锂限';
end
...
title(sprintf('图6: 单电机连通可行性边界 - 电器限/析锂限 (T_{init}=%.0f°C, T_{amb}=%.0f°C, SOC_0=%.0f%%)', ...
...
figure(66); clf;
set(gcf, 'Position', [300 70 1120 760], 'Color', 'w');
hold on;
dTdt_raw_plot = min(max(dTdt_raw_map, p.fig6_dTdt_clim(1)), p.fig6_dTdt_clim(2));
contourf(F_B, L_B*1000, dTdt_raw_plot, ...
    0:0.2:p.fig6_dTdt_clim(2), ...
    'LineColor', 'none');
colormap(gca, parula(256));
cb = colorbar; ylabel(cb, '原始初始温升速率 dT/dt (°C/min)');
clim(p.fig6_dTdt_clim);
contour(F_B, L_B*1000, dTdt_raw_plot, ...
    0.2:0.2:p.fig6_dTdt_clim(2), ...
    '-', 'Color', [0.55 0.55 0.55], 'LineWidth', 0.45);
h_lines_raw = gobjects(1, 7);
leg_labels_raw = cell(1, 7);
n_lines_raw = 0;
if min(electrical_limit_map(:)) <= 1.0 && max(electrical_limit_map(:)) >= 1.0
    [~, h_elec_raw] = contour(F_B, L_B*1000, electrical_limit_map, [1.0 1.0], ...
        '-', 'Color', [1.00 0.05 0.05], 'LineWidth', 3.0);
    n_lines_raw = n_lines_raw + 1;
    h_lines_raw(n_lines_raw) = h_elec_raw;
    leg_labels_raw{n_lines_raw} = '电器限';
end
if min(plating_limit_map(:)) <= 1.0 && max(plating_limit_map(:)) >= 1.0
    [~, h_plating_raw] = contour(F_B, L_B*1000, plating_limit_map, [1.0 1.0], ...
        '-', 'Color', [1.00 0.00 1.00], 'LineWidth', 3.0);
    n_lines_raw = n_lines_raw + 1;
    h_lines_raw(n_lines_raw) = h_plating_raw;
    leg_labels_raw{n_lines_raw} = '析锂限';
end
if max(dTdt_raw_map(:)) >= 1.0 && min(dTdt_raw_map(:)) <= 1.0
    [~, h4_raw] = contour(F_B, L_B*1000, dTdt_raw_map, [1.0 1.0], ...
        '--', 'Color', [0.00 0.95 0.15], 'LineWidth', 2.8);
    n_lines_raw = n_lines_raw + 1;
    h_lines_raw(n_lines_raw) = h4_raw;
    leg_labels_raw{n_lines_raw} = '1°C/min';
end
if max(dTdt_raw_map(:)) >= 0.5 && min(dTdt_raw_map(:)) <= 0.5
    [~, h5_raw] = contour(F_B, L_B*1000, dTdt_raw_map, [0.5 0.5], ...
        '--', 'Color', [0.00 0.95 1.00], 'LineWidth', 2.6);
    n_lines_raw = n_lines_raw + 1;
    h_lines_raw(n_lines_raw) = h5_raw;
    leg_labels_raw{n_lines_raw} = '0.5°C/min';
end
h6_raw = plot(p.f_sw, p.L_d*1000, 'wp', 'MarkerSize', 16, ...
    'MarkerFaceColor', [1 1 1], 'LineWidth', 2.4);
n_lines_raw = n_lines_raw + 1;
h_lines_raw(n_lines_raw) = h6_raw;
leg_labels_raw{n_lines_raw} = '当前';
h_limit6_raw = xline(p.f_control_max, ':', sprintf('  堵转载频上限 %.0fHz', p.f_control_max), ...
    'Color', [1 1 1], 'LineWidth', 1.8, 'LabelOrientation', 'horizontal', ...
    'LabelVerticalAlignment', 'bottom');
n_lines_raw = n_lines_raw + 1;
h_lines_raw(n_lines_raw) = h_limit6_raw;
leg_labels_raw{n_lines_raw} = 'f上限';
h7_raw = yline(p.L_d*1000, '-', sprintf('  L_d=%.1fuH', p.L_d*1e6), ...
    'LineWidth', 2.0, 'Color', [1 1 1]);
n_lines_raw = n_lines_raw + 1;
h_lines_raw(n_lines_raw) = h7_raw;
leg_labels_raw{n_lines_raw} = sprintf('L_d %.1fuH', p.L_d*1e6);
h_lines_raw = h_lines_raw(1:n_lines_raw);
leg_labels_raw = leg_labels_raw(1:n_lines_raw);
xlabel('开关频率 f (Hz)'); ylabel('D轴电感 L_d (mH)');
title(sprintf('图6b: 单电机原始温升能力背景 + 电器限/析锂限 (T_{init}=%.0f°C, T_{amb}=%.0f°C, SOC_0=%.0f%%)', ...
    p.T_init, p.T_amb, p.SOC_init*100), 'FontWeight', 'bold');
xlim([min(f_bnd), max(f_bnd)]);
ylim(p.fig6_L_display_mH);
set(gca, 'XTick', [100 500 1000 1500 2000 2500 3000]);
set(gca, 'YTick', [0.05 0.08 0.10 0.12 0.15 0.20 0.25 0.30]);
lgd6_raw = legend(h_lines_raw, leg_labels_raw, 'Location', 'southwest', 'FontSize', 8, ...
    'TextColor', 'w', 'Color', [0.18 0.18 0.18], 'EdgeColor', [0.18 0.18 0.18]);
lgd6_raw.ItemTokenSize = [24 10];
grid on;
set(gca, 'GridLineStyle', '--', 'GridColor', [0.55 0.55 0.55], ...
    'GridAlpha', 0.35, 'LineWidth', 1.1, 'Layer', 'top', 'Box', 'on');
hold off;
...
fprintf('\n图1、图2a/2b/2c、图3-图6、图6b已生成。\n');
fprintf('口径: 公共电池/直流母线, 单条电机支路连通; 电池发热按整车电池组总电流, 电池侧限流模式=%s。\n', ...
    p.battery_current_limit_mode);
fprintf('=== 完成 ===\n');
...
% 计算给定电机数量下的单个工作点。
    ...
    current_window = get_current_window_limit(p, T_celsius, SOC, R_branch_equiv);
    ...
    if apply_limits
        ...
        if current_window.enabled_as_limit
            if isfinite(current_window.charge_peak) && I_pack_min_raw < 0
                scale_candidates(end+1) = current_window.charge_peak / abs(I_pack_min_raw);
            end
            if isfinite(current_window.discharge_peak) && I_pack_max_raw > 0
                scale_candidates(end+1) = current_window.discharge_peak / I_pack_max_raw;
            end
        end
    ...
    % 这里把P_bat定义为电池公共内阻上的焦耳热; 单电机连通时该功率由单支路电流决定。
    P_bat = I_pack_rms^2 * R_bat_eff;
    % 外部电器损耗只用于总电耗和热损耗分解, 不并入电池0D热容。
    % 默认采用研发给出的平均效率口径, 避免继续沿用不准的器件级默认损耗参数。
    if p.use_average_loss_model
        [P_motor, P_inv] = calc_average_external_losses(p, P_bat);
    else
        P_motor = motor_count * I_branch_rms^2 * R_s_eff;
        P_inv = motor_count * calc_inverter_loss(p, I_branch_rms, p.f_sw, p.V_pack_nom);
    end
    ...
    op.I_spec_charge_limit = current_window.spec_charge_peak;
    op.I_spec_discharge_limit = current_window.spec_discharge_peak;
    op.I_plating_charge_limit = current_window.plating_charge_peak;
    op.current_limit_model = current_window.model;
...
% 汇总单电机连通时域仿真的能量分配, 用于图5。
    ...
    energy.values = [E_stored, E_loss, E_motor, E_inv] / 1e6;
    energy.percent = energy.values / max(sum(energy.values), eps) * 100;
    energy.labels = { ...
        sprintf('有效储能 %.2fMJ (%.1f%%)', energy.values(1), energy.percent(1)), ...
        sprintf('散热 %.2fMJ (%.1f%%)', energy.values(2), energy.percent(2)), ...
        sprintf('电机等效损耗 %.2fMJ (%.1f%%)', energy.values(3), energy.percent(3)), ...
        sprintf('控制器等效损耗 %.2fMJ (%.1f%%)', energy.values(4), energy.percent(4))};
    energy.total_MJ = E_total / 1e6;
    energy.eta_sys = E_stored / max(E_total, eps) * 100;
end
...
function current_window = get_current_window_limit(p, T_celsius, SOC, R_branch_equiv)
% 电池侧限流模型。规格书窗口保留为参考, 高频模式使用析锂自适应回充限值。

    limit_params = build_3806_battery_limit_params(p, R_branch_equiv, p.L_d);

    [current_window, ~] = battery_current_limit_model( ...
        p.f_sw, T_celsius, SOC, p.N_parallel, p.C_cell, limit_params);
end
...
% 打印工作点。
    ...
    fprintf('电池限流: 回充/放电 %.0f/%.0fA (%s, %s)\n', ...
        op.I_charge_limit, op.I_discharge_limit, window_text, op.current_limit_model);
    fprintf('参考边界: 规格书回充/放电 %.0f/%.0fA, 高频析锂回充 %.0fA\n', ...
        op.I_spec_charge_limit, op.I_spec_discharge_limit, op.I_plating_charge_limit);
end
...
% 参数完整性检查。
    ...
    valid_limit_modes = {'off', 'spec_window', 'plating_adaptive'};
    if ~any(strcmpi(p.battery_current_limit_mode, valid_limit_modes))
        error('battery_current_limit_mode只能为 off/spec_window/plating_adaptive。');
    end
    plating_fields = {'plating_R_ct_ref', 'plating_Ea_ct', 'plating_R_SEI_ref', ...
        'plating_Ea_SEI', 'plating_C_dl', 'plating_k_safety'};
    for k = 1:numel(plating_fields)
        if ~isfield(p, plating_fields{k}) || ~isfinite(p.(plating_fields{k})) || p.(plating_fields{k}) <= 0
            error('缺少有效析锂模型参数 %s。', plating_fields{k});
        end
    end
