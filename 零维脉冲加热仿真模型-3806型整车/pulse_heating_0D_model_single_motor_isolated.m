...
pie(max(single_energy.report.values, 0.001), single_energy.report.labels);
title({sprintf('图5: %.0fmin能量分配 (显式损耗主结果)', p.t_end_min), ...
    sprintf('总电耗=%.2fMJ, 显式加热效率=%.1f%%', ...
    single_energy.report.total_MJ, single_energy.report.eta_sys)}, ...
...
for k = 1:numel(T_eff_scan)
    ...
    eta_single(k) = op2.P_bat / max(P_total_2, eps) * 100;
    soc_per_deg_single(k) = (P_total_2 / (p.E_pack_kWh * 3.6e6)) / ...
